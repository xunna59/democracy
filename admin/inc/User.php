...
class User{
	...
	public function register($email, $name, $password, $state){
		require('connect.php');
		$newPassword = md5($password);
		$check_email = mysqli_query($db, "SELECT email FROM admin where Email = '$email' ");
		$check_state = mysqli_query($db, "SELECT state FROM admin where state = '$state' ");

if(mysqli_num_rows($check_email) > 0){
    echo '<script> alert("Email already registered");
	window.location.href = "reg-admin";
	 </script>';
}
if(mysqli_num_rows($check_state) > 0){
    echo '<script> alert("Selected State Admin already registered");
	window.location.href = "reg-admin";
	 </script>';
}
		...
		$query = "INSERT INTO admin (email,name,password,state,verified) value ('$email','$name','$newPassword','$state','1')";
	...
	public function update($email, $name){
		...
		$query = "UPDATE admin set email='$email', name='$name' where id= '$id'";
		$run = $sonawap->query($query) or die($sonawap->error.__LINE__);
	...
	public function changePassword($oldPassword, $newPassword){
	    ...
	    $query = "SELECT * FROM admin where id = '$user_id' and password = '$oldPassword'  ";
	    $result = $sonawap->query($query) or die($sonawap->error.__LINE__);
	    $countpassword = $result->num_rows;
	    if ($countpassword > 0) {
	      	$query1 = "UPDATE admin SET password = '$newPassword' where id = $user_id ";
	...
	public function changepin($oldpin, $newpin){
	    require 'connect.php';
	    $user_id = $_SESSION['user_id'];
	    $query = "SELECT * FROM admin where id = '$user_id' and pin = '$oldpin'  ";
	    $result = $sonawap->query($query) or die($sonawap->error.__LINE__);
	    $countpin = $result->num_rows;
	    if ($countpin > 0) {
	      	$query1 = "UPDATE admin SET pin = '$newpin' where id = $user_id ";
	      	$sendMessage = $sonawap->query($query1) or die($sonawap->error.__LINE__);
	      	if ($sendMessage) {
