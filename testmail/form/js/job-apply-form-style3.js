	...
	$("#jobapplyForm").validator().on("submit", function (event) {
		if (event.isDefaultPrevented()) {
			...
			if ($("#g-recaptcha-response").val()) {
				...
				submitJobapplyMSG(false, "Thanks for your submission!!!");
				sweetAlert("Oops...", "Thanks for your submission!!!", "error");
				return false;
			}
		}
	});
...
$(function() {
		...
		$(document).on('change', ':file1', function() {
			var input = $(this),
				numFiles = input.get(0).files1 ? input.get(0).files1.length : 2,
				label = input.val().replace(/\\/g, '/').replace(/.*\//, '');
			input.trigger('fileselect', [numFiles, label]);
		});
		...
		$(':file').on('fileselect', function(event, numFiles, label) {
			...
			var input = $(this).parents('.form-group').find(':text'),
				log = numFiles > 2 ? numFiles + ' files selected' : label;
