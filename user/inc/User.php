...
class User{
	...
	$activeid = $_SESSION['user_id'];
	 $activestatus = "UPDATE lgausers SET status = 'Active' where id = '$activeid'";
    $proceed = $sonawap->query($activestatus) or die($sonawap->error.__LINE__);
 	...
 	public function checkAuth(){
  	...
  	public function logout(){
  		require 'connect.php';
  			...
  			$inactiveid = $_SESSION['user_id'];
  		$inactivestatus = "UPDATE lgausers SET status = 'Inactive' where id = '$inactiveid'";
    $logoff = $sonawap->query($inactivestatus) or die($sonawap->error.__LINE__);
    if ($logoff == true) {
    	session_destroy();
