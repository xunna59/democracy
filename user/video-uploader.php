...
$videoName = $lga. " LGA - " . date('Y.m.d') . " - " . date("h.i.sa") . '.mp4';
 move_uploaded_file($tmpName, 'videos/' . $videoName);
    	...
    	$result = mysqli_query($db, "INSERT INTO videos (userid, email, state, lga, ward, polling_unit, videolink, datecreated) 

          VALUES ('$fullname', '$email', '$state', '$lga', '$ward', '$polling_unit', '$videoName', '$date')");
